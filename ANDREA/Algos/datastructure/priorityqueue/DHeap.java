...
public class DHeap<K extends Comparable<K>,D> implements PriorityQueue<K,D>{
	...
	*/
	public D findMin() {
		return this.isEmpty() ? this.nodes.get(0).data : null;
	}
	...
	*/
	public void delete(PriorityQueueNode<K,D> node) {
		...
		DHeapNode<K,D> heapNode = (DHeapNode<K, D>) node;
		int i = heapNode.index;
		DHeapNode<K,D> u = this.nodes.remove(n - 1);
		this.n--;
	...
	*/
	public void deleteMin() {
		DHeapNode<K, D> root = this.nodes.get(0);
		delete(root);
	}
	...
	*/	
	public void increaseKey(K newKey, PriorityQueueNode<K,D> node) {
		DHeapNode<K,D> heapNode = (DHeapNode<K,D>)node;
		if (newKey.compareTo(heapNode.getKey()) > 0){
			heapNode.key = newKey;
			moveDown(heapNode);
		}
	}
